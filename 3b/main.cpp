...
int main()
{
    ...
    int totalSum = 0;
    int currRow = 0;
    std::map<std::tuple<int, int>, std::vector<int>> gearLocations;
    for (const std::string &data : lines)
    {
        State currentState = NOT_A_NUMBER;
        int currCol = 0;
        std::string buffer;
        std::optional<std::tuple<int, int>> gearLocation;
        for (char c : data)
        {
            ...
            for (std::tuple<int, int> offset : offsets)
            {
                ...
                if (bounds_check(rowToCheck, colToCheck))
                {
                    ...
                    if (elem == '*')
                    {
                        ...
                        gearLocation = std::make_tuple(colToCheck, rowToCheck);
                        break;
                    }
                }
            }
            ...
            {
                auto unwrappedGearLocation = gearLocation.value();
                int number = std::stoi(buffer);
                if (gearLocations.find(unwrappedGearLocation) != gearLocations.end())
                {
                    gearLocations[unwrappedGearLocation].push_back(number);
                }
                else
                {
                    gearLocations[unwrappedGearLocation] = {number};
                }
            }
            currCol++;
        }
    ...
    for (const auto &pair : gearLocations)
    {
        if (pair.second.size() == 2)
        {
            int gearRatio = pair.second[0] * pair.second[1];
            totalSum += gearRatio;
        }
    }
